...
<xsl:template match="//argshere[@idtype='app_server' and @actype='configure' and @type='current']">
<xsl:text> 
</xsl:text>
</xsl:template>
...
datasource.url                  jdbc:mysql://<xsl:value-of select="//instances/instance[@name='DB']/target"/>/rubis
