...
<xsl:template match="//argshere[@id='MYSQL_ADMIN_MON_ignition.sh' and @type='current']"> $SMI $EMI $WORKLOAD &amp; </xsl:template>

<xsl:template match="//argshere[@id='MYSQL_ADMIN_MON_ignition.sh' and @type='pre-action']">
<xsl:text> 
</xsl:text>
</xsl:template>


<xsl:template match="//argshere[@id='MYSQL_ADMIN_MON_ignition.sh' and @type='post-action']"> 
<xsl:text> 
</xsl:text>
</xsl:template>
